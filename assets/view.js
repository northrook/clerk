...
class BlueScreen
{
  static init( ajax )
  {
    ...
    let blueScreen = document.getElementById( 'exception-view' )

    document.documentElement.classList.add( 'exception-visible' )
    ...
    blueScreen.addEventListener( 'tracy-toggle', ( e ) => {
      if ( e.target.matches( '#tracy-bs-toggle' ) ) { // blue screen toggle
        document.documentElement.classList.toggle( 'exception-visible', !e.detail.collapsed )
    ...
    if ( document.documentElement.classList.contains( 'exception-visible' ) ) {
      window.scrollTo( 0, 0 )
    }
  }
  ...
  {
    let ajaxBs = document.getElementById( 'exception-view' )
    if ( ajaxBs ) {
      ajaxBs.remove()
    }
    document.body.insertAdjacentHTML( 'beforeend', content )
    ajaxBs = document.getElementById( 'exception-view' )
...
{
  let footer = document.querySelector( '#exception-view footer' )
