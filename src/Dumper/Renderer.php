...
{
    private const string TypeArrayKey = 'array';
    ...
    public function renderAsHtml( stdClass $model ) : string
    {
        try {
            ...
            if ( false === $this->lazy ) { // no lazy-loading
        ...
        if ( $model->location && $this->sourceLocation ) {
            ...
            $location             = Helpers::formatHtml(
                '<a href="%" class="tracy-dump-location" target="_top" title="in file % on line %%">',
                $uri ?? '#',
                $file,
                $line,
        ...
        return '<pre class="tracy-dump'.( $this->theme ? ' tracy-'.\htmlspecialchars( $this->theme ) : '' )
               .( $json && true === $this->collapseTop ? ' tracy-collapsed' : '' ).'"'
               .( null !== $snapshot ? " data-tracy-snapshot='".self::jsonEncode( $snapshot )."'" : '' )
    ...
    {
        return match ( true ) {
            null === $value => '<span class="tracy-dump-null">null</span>',
            \is_bool(
                $value,
            ) => '<span class="tracy-dump-bool">'.( $value
                            ? 'true' : 'false' ).'</span>',
            \is_int(
                $value,
            ) => '<span class="tracy-dump-number">'.$value.'</span>',
            \is_float(
                $value,
            ) => '<span class="tracy-dump-number">'.self::jsonEncode(
                $value,
            ).'</span>',
            \is_string( $value ) => $this->renderString(
                $value,
                $depth,
                $keyType,
            ),
            \is_array( $value ), Value::TypeArray === $value->type => $this->renderArray(
                $value,
                $depth,
            ),
            Value::TypeRef === $value->type => $this->renderVar(
                $this->snapshot[$value->value],
                $depth,
                $keyType,
            ),
            Value::TypeObject === $value->type => $this->renderObject(
                $value,
                $depth,
            ),
            Value::TypeNumber === $value->type => '<span class="tracy-dump-number">'.Helpers::escapeHtml(
                $value->value,
            ).'</span>',
            Value::TypeText === $value->type => '<span class="tracy-dump-virtual">'.Helpers::escapeHtml(
                $value->value,
            ).'</span>',
            Value::TypeStringHtml === $value->type, Value::TypeBinaryHtml === $value->type => $this->renderString(
                $value,
                $depth,
                $keyType,
            ),
            Value::TypeResource === $value->type => $this->renderResource(
                $value,
                $depth,
            ),
    ...
    private function renderString( string|Value $str, int $depth, string|int|null $keyType ) : string
    {
        ...
        $unit  = Value::TypeStringHtml === $str->type ? 'characters' : 'bytes';
        ...
        if ( $count ) {
            ...
            if ( $depth ) {
                ...
                $toggle    = '<span class="tracy-toggle'.( $collapsed ? ' tracy-collapsed'
                                : '' ).'">string</span>'."\n";
            }
    ...
    private function renderArray( array|Value $array, int $depth ) : string
    {
        ...
        elseif ( null === $array->items ) {
            ...
            if ( $array->id && ( $array->depth < $depth || isset( $this->above[$array->id] ) ) ) {
                if ( false !== $this->lazy ) {
                    $ref = new Value( Value::TypeRef, $array->id );
                    $this->copySnapshot( $ref );
                    return '<span class="tracy-toggle tracy-collapsed" data-tracy-dump=\''.\json_encode(
                        $ref,
                    )."'>".$out.'</span>';
                }
                if ( $this->hash ) {
    ...
    private function renderObject( Value $object, int $depth ) : string
    {
        ...
        $out = '<span class="tracy-dump-object"'.$editorAttributes.'>'
               .( $pos
                        ? Helpers::escapeHtml( \substr( $object->value, 0, $pos + 1 ) ).'<b>'.Helpers::escapeHtml(
                            \substr( $object->value, $pos + 1 ),
                        ).'</b>'
        ...
        if ( $object->id && isset( $this->parents[$object->id] ) ) {
            return $out.' <i>RECURSION</i>';
        }
        if ( $object->id && ( $object->depth < $depth || isset( $this->above[$object->id] ) ) ) {
            if ( false !== $this->lazy ) {
        ...
        $collapsed = $object->collapsed ?? ( $depth
                ? ( false === $this->lazy || 1 === $depth ? \count( $object->items ) >= $this->collapseSub : true )
                : ( \is_int( $this->collapseTop ) ? \count( $object->items ) >= $this->collapseTop
                        : $this->collapseTop ) );
    ...
    private function renderResource( Value $resource, int $depth ) : string
    {
        ...
        if ( isset( $this->above[$resource->id] ) ) {
            if ( false !== $this->lazy ) {
        ...
        $out
                = "<span class=\"tracy-toggle tracy-collapsed\">{$out}</span>\n<div class=\"tracy-collapsed\">";
    ...
    private function copySnapshot( mixed $value ) : void
    {
        ...
        if ( null === $this->snapshotSelection ) {
        ...
        elseif ( $value instanceof Value && Value::TypeRef === $value->type ) {
    ...
    public static function jsonEncode( mixed $value ) : string
    {
        ...
        try {
            return \json_encode(
                $value,
                JSON_HEX_APOS | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES,
            );
        }
        finally {
            if ( false !== $old ) {
