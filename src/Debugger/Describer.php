...
use Closure;
use Northrook\Dumper\{Dumper, Exposer, Value};
...
final class Describer
{
    ...
    private const int JsSafeInteger = 1 << 53 - 1;
    ...
    /** @var null|(callable(string, mixed): bool) */
    public $scrubber;
    ...
    public function describe( mixed $var ) : stdClass
    {
        \uksort(
            $this->objectExposers,
            fn( $a, $b ) : int => '' === $b || ( \class_exists( $a, false ) && \is_subclass_of( $a, $b ) ) ? -1 : 1,
        );
    ...
    private function describeArray( array $arr, int $depth = 0, ?int $refId = null ) : Value|array
    {
        if ( $refId ) {
            ...
            if ( $depth && $this->maxItems && \count( $arr ) > $this->maxItems ) {
                $value->length = \count( $arr );
    ...
    private function describeObject( object $obj, int $depth = 0 ) : Value
    {
        ...
        if ( $this->location ) {
            ...
            if ( $rc->getFileName() && ( $editor = Helpers::editorUri( $rc->getFileName(), $rc->getStartLine() ) ) ) {
                $value->editor
                        = (object) ['file' => $rc->getFileName(), 'line' => $rc->getStartLine(), 'url' => $editor];
            }
        }
        ...
        if ( $this->maxDepth && $depth < $this->maxDepth ) {
            ...
            foreach ( $props ?? [] as $k => $v ) {
                $this->addPropertyTo(
                    ...
                    (string) $k,
                    $v,
                    Value::PropertyVirtual,
                    $this->getReferenceId( $props, $k ),
                );
            }
        }
    ...
    {
        if ( \preg_match( '#^[\w!\#$%&*+./;<>?@^{|}~-]{1,50}$#D', $key ) && ! \preg_match(
            '#^(true|false|null)$#iD',
            $key,
        ) ) {
            return $key;
    ...
    public function addPropertyTo(
        ...
        $value->items[] = [
            $this->describeKey( $k ),
            Value::PropertyVirtual !== $type && $this->isSensitive( $k, $v, $class )
                    ? new Value( Value::TypeText, self::hideValue( $v ) )
                    : ( $described ?? $this->describeVar( $v, $value->depth + 1, $refId ) ),
            Value::PropertyPrivate === $type ? $class : $type,
        ] + ( $refId ? [3 => $refId] : [] );
    }
    ...
    private function isSensitive( string $key, mixed $val, ?string $class = null ) : bool
    {
        return $val instanceof SensitiveParameterValue
               || ( null !== $this->scrubber && ( $this->scrubber )( $key, $val, $class ) )
    ...
    public function describeEnumProperty( string $class, string $property, mixed $value ) : ?Value
    {
        ...
        $constants
                = \array_map(
                    fn( string $const ) : string => \str_replace( "{$class}::", 'self::', $const ),
                    $constants,
                );
    ...
    private static function findLocation() : ?array
    {
        foreach ( \debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS ) as $item ) {
            if ( isset( $item['class'] ) && ( self::class === $item['class'] || Dumper::class === $item['class'] ) ) {
                $location = $item;
        ...
        if ( isset( $location['file'], $location['line'] ) && @\is_file(
            $location['file'],
        ) ) { // @ - may trigger error
