...
final class ExceptionView
{
    ...
    /** @var null|callable a callable returning true for sensitive data; fn(string, mixed): bool */
    public $scrubber;
    ...
    public function __construct()
    {
        ...
        $this->collapsePaths
                = \preg_match( '#(.+/vendor)/tracy/tracy/src/Tracy/BlueScreen$#', \strtr( __DIR__, '\\', '/' ), $m )
    ...
    public function render( Throwable $exception ) : void
    {
        ...
        $this->renderTemplate( $exception, __DIR__.'/ErrorHandler/page.phtml' );
    }
    ...
    public function renderToAjax( Throwable $exception, DeferredContent $defer ) : void
    {
        try {
            $defer->addSetup(
                ...
                Debugger::capture(
                    fn() => $this->renderTemplate( $exception, __DIR__.'/assets/content.phtml' ),
                ),
            );
        }
        catch ( Throwable $e ) {
    ...
    public function renderToFile( Throwable $exception, string $file ) : bool
    {
        if ( $handle = @\fopen( $file, 'x' ) ) {
            ...
            \ob_start(
                function( $buffer ) use ( $handle ) : void {
                    \fwrite( $handle, $buffer );
                },
                4_096,
            );
            $this->renderTemplate( $exception, __DIR__.'/ErrorHandler/page.phtml', toScreen : false );
            \ob_end_flush();
            \ob_end_clean();
            \fclose( $handle );
            return true;
    ...
    private function renderTemplate( Throwable $exception, string $template, bool $toScreen = true ) : void
    {
        ...
        if ( \function_exists( 'apache_request_headers' ) ) {
            ...
            $httpHeaders
                         = \array_filter( $_SERVER, fn( $k ) => \strncmp( $k, 'HTTP_', 5 ) === 0, ARRAY_FILTER_USE_KEY );
            $httpHeaders = \array_combine(
                \array_map(
                    fn( $k ) => \strtolower( \strtr( \substr( $k, 5 ), '_', '-' ) ),
        ...
        $nonceAttr = $toScreen ? Debugger::getNonceAttr() : null;
    ...
    private function renderPanels( ?Throwable $ex ) : array
    {
        ...
        foreach ( $this->panels as $callback ) {
            ...
            $res[] = (object) [
                ...
                'panel' => \nl2br( Escape::html( (string) $throwable ) ),
            ];
        }
    ...
    private function renderActions( Throwable $ex ) : array
    {
        ...
        if ( \preg_match( '# ([\'"])(\w{3,}(?:\\\\\w{2,})+)\1#i', $ex->getMessage(), $m ) ) {
            $class = $m[2];
            if (
                ! \class_exists( $class, false ) && ! \interface_exists( $class, false ) && ! \trait_exists(
                    $class,
                    false,
                )
    ...
    public static function highlightFile(
        ...
        $source = $php
                ...
                : '<pre class=tracy-code><div>'.CodeHighlighter::highlightLine(
                    \htmlspecialchars( $source, ENT_IGNORE, 'UTF-8' ),
                    $line,
                    $column,
                ).'</div></pre>';
        ...
        if ( $editor = Helpers::editorUri( $file, $line ) ) {
            $source = \substr_replace(
                $source,
                ' title="Ctrl-Click to open in editor" data-tracy-href="'.Helpers::escapeHtml( $editor ).'"',
                4,
                0,
            );
        }
    ...
    {
        return fn( $v, $k = null ) : string => Dumper::toHtml(
            $v,
            [
    ...
    {
        $msg = Helpers::encodeString(
            \trim( (string) $exception->getMessage() ),
        ...
        $msg = \preg_replace_callback(
            ...
            function( $m ) {
                ...
                return '<a href="'.Helpers::escapeHtml(
                    Helpers::editorUri( $r->getFileName(), $r->getStartLine() ),
                ).'" class="tracy-editor">'.$m[0].'</a>';
            },
            $msg,
        );
        ...
        $msg = \preg_replace_callback(
            ...
            fn( $m ) => @\is_file( $m[1] ) // @ - may trigger error
                        ? '<a href="'.Helpers::escapeHtml(
                            Helpers::editorUri( $m[1], isset( $m[2] ) ? (int) $m[2] : null ),
                        ).'" class="tracy-editor">'.$m[0].'</a>'
                        : $m[0],
            $msg,
        );
