...
class Bar
{
    ...
    public function render( DeferredContent $defer ) : void
    {
        ...
        elseif ( Helpers::isHtmlMode() ) {
            ...
            //     Debugger::log(
            //         new LogicException(
            //             'Tracy cannot display the Bar because the Content-Length header is being sent',
            //         ),
            //         Debugger::EXCEPTION,
            //     );
            // }
    ...
    private function renderPartial( string $type, string $suffix = '' ) : array
    {
        ...
        return [
            'bar' => Helpers::capture(
                function() use ( $type, $panels ) {
                    require __DIR__.'/assets/bar.phtml';
                },
            ),
            'panels' => Helpers::capture(
                function() use ( $type, $panels ) {
    ...
    private function renderPanels( string $suffix = '' ) : array
    {
        ...
        foreach ( $this->panels as $id => $panel ) {
            ...
            try {
                ...
                $tab    = "Error in {$id}";
                $panelHtml
                        = "<h1>Error: {$id}</h1><div class='tracy-inner'>".\nl2br(
                            Helpers::escapeHtml( $e ),
                        ).'</div>';
                unset( $e );
            }
