...
use LogicException;
use Northrook\Debugger\Session\SessionStorage;
use Northrook\Helpers;
use function String\createKey;
...
final class DeferredContent
{
    ...
    public function __construct( SessionStorage $sessionStorage )
    {
        ...
        $this->requestId      = $_SERVER['HTTP_X_TRACY_AJAX'] ?? createKey();
    }
    ...
    private function buildJsCss() : string
    {
        ...
        $str = <<<'EOD'
            ...
            (function(){
            	...
            	el.textContent=
            EOD.\json_encode( Helpers::minifyCss( \implode( '', $css ) ) ).<<<EOD
            ;
    ...
    public function clean() : void
    {
        foreach ( $this->sessionStorage->getData() as &$items ) {
            ...
            $items = \array_filter( $items, fn( $item ) => isset( $item['time'] ) && $item['time'] > \time() - 60 );
        }
    }
}
